    ...
    public Map<String,Long> solution(String[] tasks) throws ParseException {
        ...
        for (int i=0;i<timeFrames.length;i++){
            ...
            if (timeFrames[i].isStop){
                ...
                if (jobCounter==0){
                    freeStart=timeFrames[i].timestamp;
                    //从忙碌开始到结束经历的时间
                    long busyTime=timeFrames[i].timestamp-busyStart;
                    maxBusyTime=Math.max(maxBusyTime,busyTime);
                }
            }else{
                //jobCounter=0,表示当前时间节点之前是空闲的，忙碌时间开始
                if (jobCounter==0){
                    busyStart=timeFrames[i].timestamp;
                    //从空闲开始到结束经历的时间
                    long freeTime=timeFrames[i].timestamp-freeStart;
